  ...
  useGesture(
    {
      onWheel: ({ event, ctrlKey, delta: [dx, dy] }) => {
        // When a card is open, let the wheel pass through so the editor
        // content can scroll natively. Only block default behavior
        // (horizontal back/forward swipe, scroll) on the canvas itself.
